...
#connect_bank = pymysql.connect("localhost","root","","db_integrasi_bank")
connect_bank = pymysql.connect("remotemysql.com","fqPBZ6CnCP","VvcjbNcVCA","fqPBZ6CnCP")
#connect_toko = pymysql.connect("localhost","root","","db_integrasi_toko")
connect_toko = pymysql.connect("remotemysql.com","JIRddJVnqj","zxbGGbsfH0","JIRddJVnqj")
...
def engineBank():
    ...
    for data in data_bank:
        ...
        if jumlah == 0:
            ...
            insert_toko = "INSERT INTO tb_invoice(id_invoice, total_transaksi, status) values(%s,%s,%s)" % (id_invoice, total_transaksi, status)
            cursor_toko.execute(insert_toko)
            connect_toko.commit()
            print("TERDAPAT PENAMBAHAN DATA PADA db_bank.tb_invoice. Pada id_invoice = %s, total_transaksi = %s, status = %s" % (id_invoice, total_transaksi, status))
